...
public class PuzzleBoardView4 extends View {

    //some number to shuffle tiles in number times
    public static final int NUM_SHUFFLE_STEPS = 100;
    private PuzzleBoard4 puzzleBoard;
    private Activity activity;
    private Random random = new Random();
    int movesCount = 0;
    TextView movesTV;

    public PuzzleBoardView4(Context context) {
        super(context);
    ...
    public void initialize(Bitmap bitmap){
        ...
        //getWidth of th view
        int width = getWidth();
        puzzleBoard = new PuzzleBoard4(bitmap, width);
    }
                    ...
                    if (puzzleBoard.click(event.getX(), event.getY())) {
                        ...
                        if (Configur.isGamePlaying){
                            if (puzzleBoard.resolved()){
                                ...
                                PuzzleImageGame4x4.getPuzzleImageGame4x4().winningAlertDialog();
                            }
                        }
                        else {
