...
public class PuzzleBoardView3 extends View {

    //some number to shuffle tiles in number times
    public static final int NUM_SHUFFLE_STEPS = 100;
    private PuzzleBoard3 puzzleBoard;
    private Activity activity;
    private Random random = new Random();
    int movesCount = 0;
    TextView movesTV;

    public PuzzleBoardView3(Context context) {
        super(context);
    ...
    public void initialize(Bitmap bitmap){
        ...
        //getWidth of th view
        int width = getWidth();
        puzzleBoard = new PuzzleBoard3(bitmap, width);
    }
                    ...
                    if (puzzleBoard.click(event.getX(), event.getY())) {
                        ...
                        if (Configur.isGamePlaying){
                            if (puzzleBoard.resolved()){
                                ...
                                PuzzleImageGame3x3.getPuzzleImageGame3x3().winningAlertDialog();
                            }
                        }
                        else {
